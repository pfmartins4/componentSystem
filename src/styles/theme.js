...
const generateJsonTheme = () => {
  ...
  theme.colors = Object.keys(theme.colors).reduce(
    (acc, color) => ({
      ...acc,
      [color]: {
        light:
          theme.colors[color]?.light ||
          lighten(10, theme.colors[color].default),
        default: theme.colors[color].default,
        dark:
          theme.colors[color]?.dark || darken(10, theme.colors[color].default),
      },
    }),
    {}
  );
  theme.borders.radius.rounded =
    theme.borders.radius.rounded || sizeMultiplier(4, theme.borders.radius.lg);
